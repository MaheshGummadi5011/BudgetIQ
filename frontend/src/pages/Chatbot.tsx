...
import { Send, Bot, User, Volume2, VolumeX, Mic, MicOff, X, AlertCircle } from 'lucide-react';
...
const Chatbot = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (input.trim().length > 5000) {
      setMessages(prev => [...prev, {
        type: 'bot',
        content: '⚠️ Message is too long. Please keep it under 5000 characters.',
        timestamp: new Date()
      }]);
      return;
    }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6 relative">
          <div className="absolute top-4 right-4 text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
            <button
              onClick={() => {
